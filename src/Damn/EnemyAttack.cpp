...
void damn::EnemyAttack::Update(float t)
{
	if (_player && !_player->GetComponent<PlayerHealth>()->isDead()) {
		...
		float frontAngle = acos((float)(dir.Dot(_tr->GetForward()))) * (180.0f/(float)(PI)); //Calcula el ángulo que forma el jugador con el forward del enemigo 
		if (frontAngle > 1) { //Si el jugador está a un a más de un grado de diferencia, el enemigo sigue rotando
			float rightAngle = acos((float)(dir.Dot(_tr->GetRight()))) * (180.0f / (float)(PI)); //Usando el vector derecha del enemigo determinamos el lado hacia el que tiene que rotar
